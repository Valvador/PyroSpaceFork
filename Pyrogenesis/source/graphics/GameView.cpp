...
//			ResetCamera();
	...
	// New Mouse movement logic that should be similar to Homeworld 2.
	if (HotkeyIsPressed("camera.rotation"))
	{
		// Using raw mouse_dx and mouse_dy and multiplying by rotate speed is too fast.
		// Rotation Axes reversed intentionally, ViewRotateYSpeed * mouse_dx leads to proper orientation.
		// TODO: Unconfuse the mouse_dx orientation and RotateY.
		// TODO: Prevent mouse from changing position when rotating.
		m->RotateY.AddSmoothly(m->ViewRotateYSpeed * mouse_dx * deltaRealTime);  
		m->RotateX.AddSmoothly(m->ViewRotateXSpeed * mouse_dy * deltaRealTime); 
	}
	...
	// Modified default.cfg to have a different ViewRotateXmin and ViewRotateXmas (-80, 80)
	if (m->ConstrainCamera)
